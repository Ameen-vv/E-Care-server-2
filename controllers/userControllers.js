    ...
    userModel.findOne({ email: email }).then((user) => {
      if (user) {
        if (!user.block) {
          ...
          res.status(200).json({
            ok: false,
            message: "user is blocked by site",
          });
        }
      } else {
        res.status(200).json({
          ok: false,
          message: "User with this email dont exist please register",
        });
      }
    });
  } catch (err) {
...
export const getDepartment = async (req, res) => {
  try {
    ...
    departmentModel
      ...
      .limit(pageNo * 4)
      .then((departments) => {
        res.status(200).json({ data:departments, count });
      });
  } catch (err) {
    res.status(500);
  }
  ...
  try {
    ...
    doctorModel.countDocuments(query).then((count) => {
      doctorModel
        ...
        .sort(sortBy)
        .then((doctors) => {
          res.status(200).json({ data:doctors, count });
        });
    });
  } catch (err) {
    res.status(500);
      ...
      ({ available, amount }) => {
        if (available) {
          appointmentModel
            ...
            .then((appointment) => {
              if (appointment) {
                res.status(200).json({ ok: false,message: 'you have already booked for the slots' });
              } else {
          ...
          res.status(200).json({ ok: false, message:'Sorry slots are full' });
        }
      }
    );
  } catch (err) {
...
export const getUserDetails = (req, res) => {
  try {
    userModel.findOne({ _id: req.userLogged }, { password: 0 }).then((user) => {
      user ? res.status(200).json({data:user}) : res.status(500);
    });
  } catch (err) {
    res.status(500);
  }
    ...
    appointmentModel.findOne({ _id: appointmentId }).then((appointment) => {
      walletModel.findOne({ userId: req.userLogged }).then((wallet) => {
        if (wallet.balance < appointment.price) {
          res.status(200).json({ payment: false,message:'Insufficient Balance' });
        } else {
            ...
            walletModel
              ...
              .then(() => {
                appointmentModel
                  ...
                  .then((update) => {
                    update.acknowledged
                      ? res.status(200).json({ payment: true,message:'Payment Successfull' })
                      : res.status(500);
                  });
              });
          });
  ...
  try {
    ...
    appointmentModel.findOne({ _id: appointmentId }).then((appointment) => {
      if(appointment.paymentStatus){
        ...
        appointmentModel.remove({_id:appointmentId}).then((remove)=>{
          res.status(200).json({ ok: true,message:'deleted' });
        })
      }
    });
